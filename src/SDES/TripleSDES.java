...
public class TripleSDES {
	...
	public static void main(String args[]) {
		...
		try {
			...
			while (true) {
				...
				if (num == 1) {
					System.out.println("Starting TripleSDES Encryption:");
					System.out.println("Enter Key 1:");
					String keynumber_1 = in.next();
	
					byte[] key1 = stringToByteArray(keynumber_1);

					System.out.println("Enter Key 2:");
					String keynumber_2 = in.next();
					byte[] key2 = stringToByteArray(keynumber_2);

					System.out.println("Enter Plaintext:");
					String plaintextNum = in.next();
					byte[] plaintext = stringToByteArray(plaintextNum);

					byte[] encryptedTripeSDES = Encrypt(key1, key2, plaintext);
					System.out
							.println("\n" + "Encrypted Value Or Ciphertext is: " + Arrays.toString(encryptedTripeSDES));
				} else if (num == 2) {
					System.out.println("Starting TripleSDES Decryption:");
					System.out.println("Enter Key 1:");
					String keynumber_1 = in.next();
					byte[] key1 = stringToByteArray(keynumber_1);
					...
					byte[] ciphertext = stringToByteArray(cipherTextNum);

					byte[] decryptedTripeSDES = Decrypt(key1, key2, ciphertext);
					System.out
							.println("\n" + "Decryption Value Or Plaintext is: " + Arrays.toString(decryptedTripeSDES));
				} else if (num == 3) {
					LoadTestCases();
				} else if (num == 4) {
	...
	public static byte[] Encrypt(byte[] key1, byte[] key2, byte[] plaintext) {
		byte[] encrypted_1 = SDESEncrypt(key1, plaintext);
		byte[] decrypted = SDESDecrypt(key2, encrypted_1);
		byte[] encrypted_2 = SDESEncrypt(key1, decrypted);
		return encrypted_2;
	}
	...
	public static void LoadTestCases() {
		System.out.println("------------------------------------------------------------------------------------");
		System.out.println("         		     Part 2 TripleSDES ");
		System.out.println("------------------------------------------------------------------------------------");
		System.out.println("  Raw Key1		Raw Key		 	Plain Text		Cipher Text   ");

		byte[][] RawKey1Set_E = new byte[4][];
		byte[][] RawKeySet_E = new byte[4][];
		byte[][] PlainText_E = new byte[4][];
		byte[][] CipherText_E = new byte[4][];

		RawKey1Set_E[0] = stringToByteArray("0000000000");
		RawKeySet_E[0] = stringToByteArray("0000000000");
		PlainText_E[0] = stringToByteArray("00000000");

		RawKey1Set_E[1] = stringToByteArray("1000101110");
		RawKeySet_E[1] = stringToByteArray("0110101110");
		PlainText_E[1] = stringToByteArray("11010111");
		
		RawKey1Set_E[2] = stringToByteArray("1000101110");
		RawKeySet_E[2] = stringToByteArray("0110101110");
		PlainText_E[2] = stringToByteArray("10101010");
		
		RawKey1Set_E[3] = stringToByteArray("1111111111");
		RawKeySet_E[3] = stringToByteArray("1111111111");
		PlainText_E[3] = stringToByteArray("10101010");

		for (int i = 0; i < 4; i++) {
			CipherText_E[i] = Encrypt(RawKey1Set_E[i], RawKeySet_E[i], PlainText_E[i]);
			System.out.println("  " + ConvertToText(RawKey1Set_E[i]) + "		" + ConvertToText(RawKeySet_E[i])
					+ "		" + ConvertToText(PlainText_E[i]) + "		" + ConvertToText(CipherText_E[i]));
		}
		
		byte[][] RawKey1Set_D = new byte[4][];
		byte[][] RawKeySet_D = new byte[4][];
		byte[][] PlainText_D = new byte[4][];
		byte[][] CipherText_D = new byte[4][];
		
		RawKey1Set_D[0] = stringToByteArray("1000101110");
		RawKeySet_D[0] = stringToByteArray("0110101110");
		CipherText_D[0] = stringToByteArray("11100110");
		
		RawKey1Set_D[1] = stringToByteArray("1011101111");
		RawKeySet_D[1] = stringToByteArray("0110101110");
		CipherText_D[1] =stringToByteArray("01010000");
		
		RawKey1Set_D[2] =stringToByteArray("0000000000");
		RawKeySet_D[2] = stringToByteArray("0000000000");
		CipherText_D[2] =stringToByteArray("10000000");
		
		RawKey1Set_D[3] =stringToByteArray("1111111111");
		RawKeySet_D[3] = stringToByteArray("1111111111");
		CipherText_D[3] =stringToByteArray("10010010");
		
		
		for (int i = 0; i < 4; i++) {
			PlainText_D[i] = Decrypt(RawKey1Set_D[i],RawKeySet_D[i], CipherText_D[i]);
			System.out.println("  " + ConvertToText(RawKey1Set_D[i]) + "		" + ConvertToText(RawKeySet_D[i])
			+ "		" + ConvertToText(PlainText_D[i]) + "		" + ConvertToText(CipherText_D[i]));
		}
	}
